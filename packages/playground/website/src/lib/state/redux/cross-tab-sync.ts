...
// Track if current update originated from a broadcast to avoid re-broadcasting
...
export function broadcastMetadataUpdate(
	...
	// Only broadcast fields that should be synced
...
/**
 * Filter changes to only include fields that should be synced across tabs.
 */
...
function handleMessage(event: MessageEvent<CrossTabMessage>): void {
	...
	if (message.type === 'metadata-update') {
		...
		try {
			// Use the updateSiteMetadata reducer action to properly merge metadata.
			// This dispatches directly to the reducer to avoid triggering OPFS write
			// (the originating tab already wrote to OPFS).
			dispatch(
