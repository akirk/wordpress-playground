...
export function handleDesktopRelayMessage(data: unknown): boolean {
	...
	if (data.type === 'desktop-relay-map') {
		const { scope, relayBaseUrl, sessionId, transport, ttl } =
		...
		desktopRelayMappings[scope] = {
			scope,
			transport: transport === 'direct' ? 'direct' : 'http',
			relayBaseUrl,
...
export async function handleDesktopRelayRequest(
	...
	if (mapping.transport === 'direct') {
		return handleDesktopDirectRelayRequest(event, mapping);
	}
...
async function handleDesktopDirectRelayRequest(
	event: FetchEvent,
	mapping: DesktopRelayMapping
) {
